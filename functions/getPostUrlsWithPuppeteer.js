...
const BOARD_PATH = '/menus/331?viewType=L&ta=ARTICLE_COMMENT&q=%EB%8F%9A%ED%95%98%EC%9D%B4&page=1&size=50';
...
exports.getPostUrlsWithPuppeteer = async (req, res) => {
  ...
  try {
    ...
    const responseMessage = `스크래핑 완료: ${successCount}개 성공, ${failedCount}개 실패 (총 ${postUrls.length}개)`;
    ...
    res.status(200).json({
      ...
      details: {
        ...
        failed: failedCount,
        sampleUrls: postUrls.slice(0, 3) // 처음 3개 URL만 응답에 포함
      }
    });
